...
int key(FILE *fp,int s[],int k[],int &vars, int &claus)
{	int v[128];
	for(int i=0;i<128;i++)
	{ 	  
	  v[i]=vars;
	  XOR_cnf(fp,s[i+64],k[i],v[i]); 
	  s[i+64]=v[i];
	  vars=vars+1;	  	
	}
	claus=claus+512;
...
int key1(FILE *fp,int s[],int k[],int &vars,int &claus)
{	int v[128];
    for(int i=0;i<128;i++)
	{
	  v[i]=vars;
	  XOR_cnf(fp,s[320-128+i],k[i],v[i]); 
	  s[320-128+i]=v[i];
	  vars=vars+1;	  	  		
	}
	claus=claus+512;
...
{	
    FILE *fp;
    fp = fopen("d:\\ tempclaus.txt","w");
    
    FILE *f;
    f = fopen("d:\\ sboxop.txt","w");
    int s[320];    
    for(int i=0;i<320;i++)
    {
       s[i]=i+1;
	...
	int vars=449,claus=0;
	key(fp,s,k,vars,claus);
	//loop for number of rounds
	for(int r=0;r<1;r++) 
    {
    	round(fp,s,vars,claus,r);
	}
	key1(fp,s,k,vars,claus);
	fprintf(fp, "Tag is:" );
	for(int i=0;i<128;i++)
	{	  
	  fprintf(fp, "\n%d %d ", s[320-128+i], 0 );	  		
